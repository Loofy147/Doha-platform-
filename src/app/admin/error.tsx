...
import { AlertCircle, RefreshCw, Home, FileText } from 'lucide-react'; // Added FileText
...
export default function AdminError({
  ...
  const handleReportError = () => {
    console.log("Simulated admin error report:", {
    ...
    toast({
      ...
      description: "شكرًا لمساعدتك في تحسين لوحة تحكم لمسة ضحى!",
      variant: "default",
    });
  };
       ...
       <Card className="w-full max-w-md shadow-xl border-destructive/50 bg-card">
        <CardHeader className="bg-destructive/10 p-6 rounded-t-lg">
          <div className="flex flex-col items-center">
            ...
            <CardTitle className="text-2xl font-semibold text-destructive">خطأ في قسم الإدارة</CardTitle>
          </div>
        </CardHeader>
        <CardContent className="p-6 space-y-4">
          <CardDescription className="text-md text-muted-foreground">
            نأسف، حدث خطأ غير متوقع أثناء عرض هذا الجزء من لوحة التحكم.
          </CardDescription>
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-3 pt-4 border-t border-border/50">
            ...
            <Button asChild>
                <Link href="/admin" className="flex items-center gap-2"> <Home className="h-4 w-4" /> العودة للوحة التحكم</Link>
            </Button>
          </div>
           <Button
              variant="link"
